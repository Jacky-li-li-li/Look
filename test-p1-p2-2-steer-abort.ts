...
class FakeAuthStorage {
	getAuthStatus(_p: string) {
		return { source: "stored" as const, label: "stored" };
	}
	set() {}
	remove() {}
	get() {
...
function makeFakeSession(opts: FakeSessionOpts = {}) {
	...
	return {
		...
		async setThinkingLevel() {
			calls.push({ method: "setThinkingLevel", args: {} });
		},
		async compact() {
...
async function main() {
	...
	await withFakeSession({ isStreaming: false }, async (m, session) => {
		...
		assert(
			promptCall?.args.options === undefined,
	...
	await withFakeSession({ isStreaming: true }, async (m, session) => {
		...
		assert(!!promptCall, "prompt was called");
		assert(
			promptCall?.args.options?.streamingBehavior === "steer",
	...
	await withFakeSession({ isStreaming: true }, async (m) => {
		let threw = false;
		try {
			await m.sendMessage("test-agent", "interrupt");
		} catch {
			threw = true;
		}
...
describe("test-p1-p2-2-steer-abort", () => {
