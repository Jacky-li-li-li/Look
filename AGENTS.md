...
- Look owns exactly one live `AgentSessionRuntime` at a time.
- Sidebar rows are pi session files discovered with `SessionManager.list`; they are not independent running agents.
- New, resume and fork operations must use `AgentSessionRuntime.newSession`, `switchSession` and `fork`.
...
- Fork replaces the one active runtime with the forked pi session. It does not create a concurrently running agent.
