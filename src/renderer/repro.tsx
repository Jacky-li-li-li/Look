...
const FAKE_AGENTS: AgentInfo[] = [
	...
	},
	{
		...
		status: "thinking",
		model: "anthropic/claude-sonnet-4-20250514",
		usage: { totalTokens: 12453, cost: { total: 0.0432 } },
	},
	{
		id: "3",
		name: "Another one",
		...
		status: "working",
		model: "anthropic/claude-sonnet-4-20250514",
		usage: { totalTokens: 88432, cost: { total: 0.21 } },
	},
	{
		id: "4",
		name: "编码器",
	...
	{
		id: "6",
		name: "Another really really long agent name that should truncate nicely",
		role: "chat",
		status: "idle",
		model: "anthropic/claude-sonnet-4-20250514",
		usage: { totalTokens: 0, cost: { total: 0 } },
				...
				<TabsList className="h-9 w-full rounded-none border-b border-hairline bg-transparent px-2">
					<TabsTrigger value="chat" className="flex-1 gap-1.5 text-[11px] data-[state=active]:bg-accent">
						...
						<Badge variant="secondary" className="ml-0.5 h-3.5 px-1 py-0 text-[9px]">
							{FAKE_AGENTS.filter(isChatAgent).length}
						</Badge>
					</TabsTrigger>
					<TabsTrigger value="orch" className="flex-1 gap-1.5 text-[11px] data-[state=active]:bg-accent">
						...
						Orch
						<Badge variant="secondary" className="ml-0.5 h-3.5 px-1 py-0 text-[9px]">
							{FAKE_AGENTS.filter(isOrchAgent).length}
						</Badge>
					</TabsTrigger>
				</TabsList>
			</Tabs>
						...
						return (
							...
							>
								...
								<div className="flex shrink-0 items-center gap-1.5">
									...
									<Button variant="line-ghost" size="icon-xs" className="opacity-100">
										<X className="size-3" />
									</Button>
								</div>
							</div>
