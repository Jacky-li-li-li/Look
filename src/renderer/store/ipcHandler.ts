...
import type { SubagentProgressEntry } from "./atoms";
...
export function markSessionSnapshotLoading(
  sessionId: string,
  loading: boolean,
): void {
...
function scheduleSharedRefresh(
  projectId: string,
  filesAtom: ReturnType<typeof sharedFilesAtomFamily>,
): void {
  ...
  sharedRefreshTimers.set(
    ...
    setTimeout(() => {
      ...
      void window.look
        ...
        .catch((error: unknown) => {
          const message =
            error instanceof Error ? error.message : "刷新共享区失败";
          toast.error(message);
        });
...
export function applySnapshot(snapshot: SessionSnapshotEnvelope): void {
  ...
  for (const entry of snapshot.entries) {
    if (
      entry.type === "custom" &&
      entry.customType === LOOK_MESSAGE_DURATION_ENTRY_TYPE
    ) {
  ...
  if (leafId) {
    ...
    if (
      ...
      leafEntry.customType === LOOK_MESSAGE_DURATION_ENTRY_TYPE
    ) {
  ...
  appStore.set(sessionStateAtomFamily(snapshot.sessionId), {
    ...
    uiBlocks:
      isAgentEnd || previous.uiPhase !== "idle" ? [] : previous.uiBlocks,
    uiTools: isAgentEnd || previous.uiPhase !== "idle" ? {} : previous.uiTools,
    uiPhase:
      isAgentEnd || previous.uiPhase !== "idle" ? "idle" : previous.uiPhase,
    uiSteering:
      isAgentEnd || previous.uiPhase !== "idle" ? [] : previous.uiSteering,
    uiFollowUp:
      isAgentEnd || previous.uiPhase !== "idle" ? [] : previous.uiFollowUp,
  ...
  for (const ev of events) {
    switch (ev.type) {
      ...
      case "assistant_text_delta": {
        textDeltas.set(
          ev.contentIndex,
          (textDeltas.get(ev.contentIndex) ?? "") + ev.delta,
        );
        break;
      }
      ...
      case "thinking_delta": {
        thinkingDeltas.set(
          ev.contentIndex,
          (thinkingDeltas.get(ev.contentIndex) ?? "") + ev.delta,
        );
        break;
      }
      case "thinking_end": {
      ...
      case "toolcall_arg_delta": {
        toolcallArgDeltas.set(
          ev.contentIndex,
          (toolcallArgDeltas.get(ev.contentIndex) ?? "") + ev.delta,
        );
        break;
      }
      case "toolcall_end": {
        ...
        const incompleteIdx = blocks.findIndex(
          (b) =>
            b.kind === "toolcall" &&
            b.contentIndex === ev.contentIndex &&
            !b.completed,
        );
        const idx =
          incompleteIdx >= 0
            ...
            : blocks.findIndex(
                (b) =>
                  b.kind === "toolcall" && b.contentIndex === ev.contentIndex,
              );
        if (idx >= 0) {
      ...
      case "assistant_message_end": {
        const mapped = blocks.map((b) =>
          b.completed ? b : { ...b, completed: true },
        );
  ...
  if (thinkingDeltas.size > 0) {
    for (let i = 0; i < blocks.length; i++) {
      ...
      if (
        b.kind === "thinking" &&
        !b.completed &&
        thinkingDeltas.has(b.contentIndex)
      ) {
        mutateBlock(i, {
          ...b,
  ...
  if (agentFlags) {
    appStore.set(
      ...
      appStore
        .get(agentsAtom)
        .map((agent) =>
          agent.id === sessionId ? { ...agent, ...agentFlags } : agent,
        ),
    );
  }
  ...
  const unsub = api.onEvent((event: MainToRendererEvent) => {
    switch (event.type) {
      ...
      case "agent:list": {
        ...
        const otherProjects = previous.filter(
          (agent) => agent.projectId !== event.projectId,
        );
        ...
        if (!sameLength) {
          ...
          if (activeId && !next.some((agent) => agent.id === activeId))
      ...
      case "agent:created":
        appStore.set(agentsAtom, [
          ...appStore
            .get(agentsAtom)
            .filter((agent) => agent.id !== event.agent.id),
          event.agent,
        ]);
        break;
      ...
      case "agent:destroyed": {
        ...
        appStore.set(
          recentlyCompletedAtom,
          appStore
            .get(recentlyCompletedAtom)
            .filter((id) => id !== event.agentId),
        );
        // Clean up opened sheet
        appStore.set(
          openedSessionIdsAtom,
          appStore
            .get(openedSessionIdsAtom)
            .filter((id) => id !== event.agentId),
        );
      ...
      case "agent:updated":
        appStore.set(
          ...
          appStore
            .get(agentsAtom)
            .map((a) => (a.id === event.agent.id ? event.agent : a)),
        );
        break;
      ...
      case "session:snapshot":
        applySnapshot(event);
        if (appStore.get(appReadyPhaseAtom) < 3)
          appStore.set(appReadyPhaseAtom, 3);
        break;
      ...
      case "project:list": {
        ...
        if (appStore.get(appReadyPhaseAtom) < 1)
          appStore.set(appReadyPhaseAtom, 1);
        const projectIds = new Set(event.projects.map((project) => project.id));
        appStore.set(
          openProjectIdsAtom,
          appStore
            .get(openProjectIdsAtom)
      ...
      case "permission:resolved": {
        appStore.set(
          ...
          appStore
            .get(permissionAskQueueAtom)
            .filter((item) => item.requestId !== event.requestId),
        );
        break;
      }
      ...
      case "plan:question-requested": {
        appStore.set(
          planQuestionRequestAtomFamily(event.agentId),
          event.request,
        );
      ...
      case "plan:question-resolved": {
        ...
        if (current?.requestId === event.requestId) {
          ...
          appStore.set(
            planQuestionDraftAtomFamily(event.agentId),
            emptyPlanQuestionDraft(),
          );
        }
        break;
      ...
      case "plan:approval-requested": {
        appStore.set(
          planApprovalRequestAtomFamily(event.agentId),
          event.request,
        );
        break;
      }
      ...
      case "usage:updated": {
        ...
        window.look
          ...
          .then(
            (result) => {
              const r = result as { success: boolean; usage?: { usage: Record<string, number>; modelCost: Record<string, Record<string, { turns: number; cost: number }>>; years: number[] }; error?: string };
      ...
      case "session:subagent-progress": {
        const atom = subagentProgressAtomFamily(event.parentSessionId);
        const current = appStore.get(atom);
        ...
        const idx = current.findIndex(
          (e) => e.childSessionId === event.childSessionId,
        );
        if (idx >= 0) {
          const next = [...current];
          ...
          appStore.set(atom, [...current, entry]);
        }
        break;
      }
      ...
      case "subagent:definitions-updated":
        void window.look
          ...
          .catch((err: unknown) => {
            console.error(
              "[ipcHandler] Failed to refresh agent definitions:",
              err,
            );
          });
        break;
...
function _autoSelectAgent(): void {
  ...
  if (
    ...
    agents.some((a) => a.id === _lastActiveSessionId)
  ) {
  ...
  void window.look
    ...
    .then((result: any) => {
      ...
      if (appStore.get(activeAgentIdAtom) === sessionId)
        appStore.set(activeAgentIdAtom, null);
    })
    .catch(() => {
      markSessionSnapshotLoading(sessionId, false);
...
export async function initAppData(api: any): Promise<void> {
  ...
  if (settingsResult?.success && settingsResult.settings) {
    ...
    if (settings.autoCollapse !== undefined)
      appStore.set(autoCollapseAtom, settings.autoCollapse);
    if (settings.preferredModel)
      appStore.set(userPreferredModelAtom, settings.preferredModel);
    if (settings.lastActiveSessionId)
      _lastActiveSessionId = settings.lastActiveSessionId;
    if (Array.isArray(settings.openProjectIds))
      appStore.set(openProjectIdsAtom, settings.openProjectIds);
    if (Array.isArray(settings.openedSessionIds))
      appStore.set(openedSessionIdsAtom, settings.openedSessionIds);
  }
  ...
  if (projectResult?.success && Array.isArray(projectResult.projects)) {
    ...
    if (projectResult.activeProjectId)
      appStore.set(activeProjectIdAtom, projectResult.activeProjectId);
  }
