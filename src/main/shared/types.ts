...
export type LookUiEvent =
	...
	| { type: "assistant_text_delta"; contentIndex: number; delta: string; timestamp: number }
	| { type: "assistant_text_end"; contentIndex: number; text: string; timestamp: number }

	// ── 思考流（assistant 消息 thinking 块） ──
	| { type: "thinking_start"; contentIndex: number; timestamp: number }
	| { type: "thinking_delta"; contentIndex: number; delta: string; timestamp: number }
	| { type: "thinking_end"; contentIndex: number; thinking: string; timestamp: number }

	// ── 工具调用流（assistant 决定调用工具） ──
	| { type: "toolcall_start"; contentIndex: number; toolCallId: string; toolName: string; timestamp: number }
	| { type: "toolcall_arg_delta"; contentIndex: number; delta: string; timestamp: number }
	| {
			type: "toolcall_end";
			contentIndex: number;
			toolCallId: string;
	...
	| { type: "tool_exec_update"; toolCallId: string; partialResult: unknown; timestamp: number }
	| {
			type: "tool_exec_end";
			toolCallId: string;
			toolName: string;
	...
	// ── 会话元数据变更 ──
	| { type: "session_meta"; field: "name" | "thinkingLevel"; value: string; timestamp: number }
