...
export function convertPiMessage(piMsg: any, agentId: string, msgId: string): AgentMessage {
  ...
  if (piRole === "assistant") {
    ...
    return {
      id: msgId, agentId, role: "assistant",
      content: blocks.filter((b: any) => b.type === "text").map((b: any) => b.text).join(""),
      thinking: blocks.filter((b: any) => b.type === "thinking").map((b: any) => b.thinking).join(""),
      toolCalls: blocks.filter((b: any) => b.type === "toolCall").map((b: any) => ({
        callId: b.id ?? "", toolName: b.name ?? "unknown", args: b.arguments ?? {},
        result: "", isError: false, status: "success" as const,
      })),
      timestamp: piMsg.timestamp ?? Date.now(),
      usage: piMsg.usage ? {
        inputTokens: piMsg.usage.input ?? 0, outputTokens: piMsg.usage.output ?? 0,
        cacheReadTokens: piMsg.usage.cacheRead ?? 0, cacheWriteTokens: piMsg.usage.cacheWrite ?? 0,
        totalTokens: piMsg.usage.totalTokens ?? 0,
        cost: { input: piMsg.usage.cost?.input ?? 0, output: piMsg.usage.cost?.output ?? 0, cacheRead: piMsg.usage.cost?.cacheRead ?? 0, cacheWrite: piMsg.usage.cost?.cacheWrite ?? 0, total: piMsg.usage.cost?.total ?? 0 },
      } : undefined,
    };
  }
