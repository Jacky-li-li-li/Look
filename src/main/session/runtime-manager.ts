...
export class SessionRuntimeManager
  implements IEventBus, IRuntimeLifecycle, ISessionEventHost
{
  ...
  private readonly runtimeInitializations = new Map<
    string,
  ...
  constructor(
    ...
    this.customProvidersStore = new CustomProvidersStore(
      this.modelRegistry,
      getCustomProvidersPath(),
    );
    ...
    this.globalSettingsManager = SettingsManager.create(
      getLookDir(),
      getLookDir(),
    );
    this.projectService = new ProjectService(
      this.trustStore,
      this.globalSettingsManager,
    );
    ...
    this.mcpManager.setOnChange(() =>
      this.emit({ type: "mcp:status-changed" }),
    );
    ...
    this.subAgentRuntimeService = new SubAgentRuntimeService(
      this,
  ...
  getWorkspaceFileService(): WorkspaceFileService {
    ...
    if (!this.workspaceFileService) {
      throw new Error(
        "WorkspaceFileService is not configured for this SessionRuntimeManager",
      );
    }
  ...
  getWorkspaceTreeService(): WorkspaceTreeService {
    ...
    if (!this.workspaceTreeService) {
      throw new Error(
        "WorkspaceTreeService is not configured for this SessionRuntimeManager",
      );
    }
  ...
  async restoreWorkspace(): Promise<number> {
    ...
    const preferredProject = this.projectService.getProjectInfo(
      settings.lastActiveProjectId,
    );
    const project = preferredProject?.valid
      ? preferredProject
      : this.listProjects().find((p) => p.valid);
    ...
    const preferred =
      sessions.find((s) => s.id === settings.lastActiveSessionId) ??
      sessions[0];
  ...
  getActiveProjectCwd(): string {
    ...
    if (!project)
      throw new Error("No active project. Select a project folder first.");
    if (!project.valid)
      throw new Error(`Project path does not exist: ${project.cwd}`);
    return project.cwd;
  }
  ...
  async createProject(
    cwd: string,
    name?: string,
  ): Promise<{ project: ProjectInfo; isDuplicate: boolean }> {
  ...
  async deleteProject(projectId: string): Promise<void> {
    ...
    const runtimeIds = Array.from(this.runtimes.entries()).flatMap(
      ([sessionId, managed]) =>
    ...
    this.emit({
      ...
      agentCount: new Set([
        ...persisted.map((session) => session.id),
        ...runtimeIds,
      ]).size,
      runningCount: runtimeIds.filter((sessionId) => {
        ...
        return Boolean(
          session &&
          (session.isStreaming || session.isRetrying || session.isCompacting),
        );
      }).length,
    });
  }
  ...
  async executeDeleteProject(projectId: string): Promise<void> {
    ...
    await Promise.all(
      runtimeIds.map((sessionId) => this.disposeRuntime(sessionId, true)),
    );
    ...
    if (this.workspaceFileService) {
      try {
        ...
        console.error(
          `[Look] Failed to stop shared area watcher for ${projectId}:`,
          error,
        );
      }
    }
    if (this.workspaceTreeService) {
      try {
        ...
        console.error(
          `[Look] Failed to stop workspace tree watchers for ${projectId}:`,
          error,
        );
      }
    }
    ...
    if (existsSync(sharedDir)) {
      try {
        ...
        if ((error as NodeJS.ErrnoException)?.code !== "ENOENT") {
          console.error(
            `Failed to remove shared area for project ${projectId}:`,
            error,
          );
        }
      }
    ...
    if (workspaceDir && existsSync(workspaceDir)) {
      try {
        ...
        if (project) {
          console.log(
            `[Look] Removed workspace for deleted project "${project.name}" (${projectId})`,
          );
        }
      } catch (error) {
        if ((error as NodeJS.ErrnoException)?.code !== "ENOENT") {
          console.error(
            `Failed to remove workspace for project ${projectId}:`,
            error,
          );
        }
      }
    }
    if (this.activeSessionId && runtimeIds.includes(this.activeSessionId))
      this.activeSessionId = null;
    if (this.projectService.activeId === projectId) {
      this.projectService.setActiveId(
        this.listProjects().find((project) => project.valid)?.id ?? null,
      );
    }
    this.saveProjects();
    this.emitSessionList(projectId);
    this.emitProjectList();
    if (this.projectService.activeId)
      this.emitSessionList(this.projectService.activeId);
  }
  ...
  private async refreshProjectSessions(
    ...
    const sessions = (
      await scanSessionDirectory(ensureWorkspaceDir(project.name), project.cwd)
    ).map((session) => ({
      ...session,
      projectId,
    }));
    ...
    if (existsSync(subsessionsDir)) {
      ...
      for (const file of subsessionFiles) {
        try {
          ...
          console.warn(
            `[Look] Failed to restore subagent session from ${file}:`,
            err,
          );
        }
      }
  ...
  private sessionInfo(session: StoredSession): AgentInfo {
    ...
    return {
      ...
      thinkingLevel:
        (piSession?.thinkingLevel as ThinkingLevel | undefined) ?? "off",
      modelSupportsThinking: piSession?.supportsThinking() ?? false,
      availableThinkingLevels: (piSession?.getAvailableThinkingLevels() as
        ThinkingLevel[] | undefined) ?? ["off"],
  ...
  private runtimeInfo(sessionId: string, managed: ManagedRuntime): AgentInfo {
    ...
    return {
      id: sessionId,
      name: (
        session.sessionManager.getSessionName() || DEFAULT_SESSION_NAME
      ).slice(0, MAX_NAME_LENGTH),
      ...
      sessionFilePath:
        session.sessionFile && existsSync(session.sessionFile)
          ? session.sessionFile
          : undefined,
  ...
  private subagentFields(
    ...
    AgentInfo,
    "parentSessionId" | "isSubagentSession" | "agentConfigName"
  > {
  ...
  listAgents(): AgentInfo[] {
    return this.listProjects().flatMap((project) =>
      this.listAgentsInProject(project.id),
    );
  }
  ...
  listAgentsInProject(projectId: string): AgentInfo[] {
    ...
    const persisted = persistedEntries.map((session) => {
      const managed = this.runtimes.get(session.id);
      return managed
        ? this.runtimeInfo(session.id, managed)
        : this.sessionInfo(session);
    });
    const persistedIds = new Set(persistedEntries.map((session) => session.id));
    const drafts = Array.from(this.runtimes.entries()).flatMap(
      ([sessionId, managed]) =>
        managed.projectId === projectId && !persistedIds.has(sessionId)
          ? [this.runtimeInfo(sessionId, managed)]
          : [],
    );
  ...
  private createRuntimeFactory(factoryOptions?: {
    ...
    return async ({ cwd, sessionManager, sessionStartEvent }) => {
      return this.withResourceInitialization(async () => {
        ...
        const sharedPath = projectId
          ? getProjectSharedDir(projectId)
          : undefined;
        ...
        const appendPrompts = [
          sharedPrompt,
          ...(factoryOptions?.appendSystemPrompt ?? []),
        ...
        const services = await createAgentSessionServices({
          ...
          resourceLoaderOptions: {
            extensionFactories: await this.buildExtensionFactories(
              cwd,
              sessionManager.getSessionId(),
            ),
            appendSystemPrompt:
              appendPrompts.length > 0 ? appendPrompts : undefined,
  ...
  private async withResourceInitialization<T>(
    task: () => Promise<T>,
  ): Promise<T> {
    ...
    try {
      ...
      release();
      if (this.resourceInitializationTail === tail)
        this.resourceInitializationTail = Promise.resolve();
    }
  }
  ...
  private async buildExtensionFactories(
    ...
    return [
      ...
      createPlanExtensionFactory(sessionId, {
        ...
        askQuestions: (id, questions, signal) =>
          this.planService.requestQuestions(id, questions, signal),
        submitPlan: (id, plan, signal) =>
          this.planService.requestApproval(id, plan, signal),
      }),
      await createSubagentExtensionFactory(
        sessionId,
        this.createSubagentHost(projectId),
        projectId,
      ),
  ...
  private async createManagedRuntime(
    ...
    const runtime = await createAgentSessionRuntime(
      this.createRuntimeFactory(factoryOptions),
      {
        cwd,
        agentDir: getLookDir(),
        sessionManager,
  ...
  private async bindRuntime(
    ...
    this.permissionService.restoreFromSession(
      session.sessionId,
      session.sessionManager,
    );
    this.planService.restoreToolSnapshot(
      session.sessionId,
      session.sessionManager,
    );
    ...
    runtime.setRebindSession(async (nextSession) =>
      this.rebindRuntime(runtime, nextSession),
    );
  ...
  private async rebindRuntime(
    runtime: AgentSessionRuntime,
    session: AgentSession,
  ): Promise<void> {
    const previousEntry = Array.from(this.runtimes.entries()).find(
    ...
    this.planService.cancelInteractions(
      previousSessionId,
  ...
  private emitRuntimeDiagnostics(
    sessionId: string,
    runtime: AgentSessionRuntime,
  ): void {
  ...
  async disposeAllRuntimes(): Promise<void> {
    await Promise.all(
      Array.from(this.runtimes.keys()).map((sessionId) =>
        this.disposeRuntime(sessionId, true),
      ),
    );
  }
  ...
  async createAgent(
    opts?:
      | { name?: string; projectId?: string; imProvider?: ImSessionProvider }
      | string,
  ): Promise<string> {
    ...
    if (!project?.valid)
      throw new Error(
        `Project path does not exist: ${project?.cwd ?? projectId}`,
      );
    ...
    session.setSessionName(
      (input.name?.trim() || DEFAULT_SESSION_NAME).slice(0, MAX_NAME_LENGTH),
    );
  ...
  private sessionManagerFor(sessionId: string): SessionManager | null {
    ...
    const stored = this.findStoredSession(sessionId);
    return stored && existsSync(stored.path)
      ? SessionManager.open(stored.path)
      : null;
  }
  ...
  async sendMessage(
    ...
    const agentTokens = Array.from(
      text.matchAll(
        /(?:^|\s)\/(?:agent|subagent):([A-Za-z0-9][A-Za-z0-9._-]*)(?=$|\s)/g,
      ),
    );
  ...
  isSubagentEnabled(sessionId: string): boolean {
    return (
      this.subagentEnabledBySession.get(sessionId) ??
      this.subagentDefaultEnabled
    );
  }
  ...
  async setSubagentEnabledGlobal(enabled: boolean): Promise<void> {
    ...
    await Promise.all(
      Array.from(this.runtimes.keys()).map((sessionId) =>
        this.applySubagentEnabled(sessionId, enabled),
      ),
    );
  }

  /** 设置单个 Agent 定义的启用状态。 */
  async setAgentDefinitionEnabled(
    name: string,
    enabled: boolean,
  ): Promise<void> {
    const settings = this.userSettings.getAll();
  ...
  private async applySubagentEnabled(
    sessionId: string,
    enabled: boolean,
  ): Promise<void> {
    ...
    if (enabled) {
      ...
      if (!active.includes("subagent"))
        session.setActiveToolsByName([...active, "subagent"]);
    } else {
   ...
   */
  private applySubagentDefaultOnBind(
    sessionId: string,
    session: AgentSession,
  ): void {
      ...
      for (const line of lines) {
        ...
        try {
          ...
          if (entry.type === "session") {
            ...
            if (entry.timestamp)
              created = new Date(String(entry.timestamp)).getTime();
          } else if (entry.type === "session_info") {
            displayName = String(entry.name ?? "") || undefined;
          } else if (
            entry.type === "custom" &&
            entry.customType === SUBAGENT_PARENT_ENTRY_TYPE
          ) {
            const data = entry.data as
              { parentSessionId?: string; agentName?: string } | undefined;
            ...
            if (!firstMessage) {
              const msg = entry.message as
                { content?: unknown; timestamp?: number } | undefined;
              ...
              if (msg?.timestamp && msg.timestamp < created)
    ...
    await Promise.all(
      Array.from(this.runtimes.values()).map((managed) =>
        managed.runtime.session.reload().catch((error) => {
          console.warn(
            "[Look][subagent] Failed to reload session after agent definition change:",
            error,
          );
        }),
      ),
  ...
  async runSubSession(
    ...
    if (!parentManaged)
      throw new Error(`Parent session ${parentSessionId} is not live`);
    ...
    if (!project?.valid)
      throw new Error(
        `Project not found or invalid for subagent: ${projectId}`,
      );
      ...
      try {
        ...
        if (slash > 0) {
          const model = this.modelRegistry.find(
            agent.model.slice(0, slash),
            agent.model.slice(slash + 1),
          );
          if (model) await session.setModel(model);
        }
      } catch (error) {
        console.warn(
          `[Look][subagent] Failed to set model ${agent.model}:`,
          error,
        );
      }
    }
  ...
  private cleanupFinalizedSubSession(childSessionId: string): void {
    // Moved to SubAgentRuntimeService; wrapper retained for internal callers.
  }

  private trackSubSessionMessageEnd(
    sessionId: string,
  ...
  async setModel(sessionId: string, modelKey: string): Promise<void> {
    ...
    if (slash <= 0)
      throw new Error(`Model key must be in provider/model form: ${modelKey}`);
    const model = this.modelRegistry.find(
  ...
  async createForkedSession(
    ...
    return this.withForkLock(sessionId, async () => {
      ...
      if (
        sourceSession.isStreaming ||
        sourceSession.isRetrying ||
        sourceSession.isCompacting
      ) {
      ...
      if (!sourceSession.sessionManager.getEntry(entryId))
      ...
      const forkManager = SessionManager.open(
        sourceFile,
        sourceSession.sessionManager.getSessionDir(),
      );
      ...
      try {
        ...
        if (opts?.name?.trim())
          session.setSessionName(opts.name.trim().slice(0, MAX_NAME_LENGTH));
        ...
        if (forkedSessionId && this.runtimes.has(forkedSessionId))
          await this.disposeRuntime(forkedSessionId, true);
  ...
  private async withForkLock<T>(
    sessionId: string,
    task: () => Promise<T>,
  ): Promise<T> {
    const previous =
      this.forkOperationTails.get(sessionId) ?? Promise.resolve();
    let release!: () => void;
    const gate = new Promise<void>((resolve) => {
      release = resolve;
    });
    ...
    try {
      ...
      release();
      if (this.forkOperationTails.get(sessionId) === tail)
   ...
   */
  private async onUserMessageEndForTitle(
    sessionId: string,
    ...
    const isDefaultName =
      (titleScope?.isDefaultName ?? false) &&
      (!currentName || currentName === DEFAULT_SESSION_NAME);
  ...
  async onAgentEnd(sessionId: string, _willRetry: boolean): Promise<void> {
    ...
    await this.refreshAfterTurn(sessionId).catch((error) =>
      this.emitError(error, sessionId),
    );
  }
  ...
  async onMessageEnd(sessionId: string, message: AgentMessage): Promise<void> {
    if (message.role === "assistant") {
      ...
      if (message.stopReason !== "aborted") {
        ...
        const cost = (message as { usage?: { cost?: { total?: number } } })
          .usage?.cost?.total;
  ...
  private async refreshAfterTurn(sessionId: string): Promise<void> {
    const projectId =
      this.runtimes.get(sessionId)?.projectId ??
      this.findStoredSession(sessionId)?.projectId;
    if (!projectId) return;
    await this.refreshProjectSessions(projectId);
    this.emitSessionUpdated(sessionId);
    this.emitSessionList(projectId);
  ...
  emitSessionState(
    ...
    reason: string = "activate",
  ): void {
    const sessionId = targetSessionId ?? this.activeSessionId;
    if (!sessionId) return;
    const managed = this.runtimes.get(sessionId);
    const projectId =
      managed?.projectId ?? this.findStoredSession(sessionId)?.projectId;
  ...
  emitSessionUpdated(sessionId: string): void {
    ...
    if (info)
      this.emit({ type: "agent:updated", agentId: info.id, agent: info });
  }
  ...
  async getProviderSettings() {
    ...
    const filtered = providers.flatMap((provider) => {
      ...
      const models = this.getAvailableModelsSync().filter(
    ...
    const customTotalModels = customList.reduce(
      (sum, cp) => sum + cp.models.length,
      0,
    );
  ...
  private async applyPermissionMode(
    ...
    if (!options.internal)
      this.planService.cancelInteractions(
        sessionId,
        "Permission mode was changed manually",
      );
    ...
    if (mode === "plan") this.planService.restrictToolsForPlan(sessionId);
      ...
      this.planService.restorePrePlanTools(sessionId);
    this.permissionService.persistIfDirty(sessionId);
    this.planService.persistToolSnapshotIfDirty(sessionId);
  ...
  async handlePlanApprovalResponse(
  ...
  async updateGeneralSettings(
    ...
    if (partial.compactionEnabled !== undefined) {
      for (const managed of this.runtimes.values()) {
        managed.runtime.session.setAutoCompactionEnabled(
          partial.compactionEnabled,
        );
      }
    }
    if (partial.permissionMode !== undefined)
      this.permissionService.setDefaultMode(partial.permissionMode);
    if (partial.subagentEnabled !== undefined) {
      this.subagentDefaultEnabled = partial.subagentEnabled;
      await Promise.all(
        Array.from(this.runtimes.keys()).map((sessionId) =>
          this.applySubagentEnabled(
            sessionId,
            partial.subagentEnabled as boolean,
          ),
        ),
      );
    }
  ...
  listSkillsForUI() {
    const activeRuntime = this.activeSessionId
      ? this.runtimes.get(this.activeSessionId)?.runtime
      : undefined;
    const skillPaths =
      activeRuntime?.services.settingsManager.getSkillPaths() ??
      this.globalSettingsManager.getSkillPaths();
  ...
  async importSkillPaths(
    ...
    try {
      ...
      const settingsManager =
        activeRuntime?.services.settingsManager ?? this.globalSettingsManager;
      const merged = Array.from(
        new Set(
          [...settingsManager.getSkillPaths(), ...paths].flatMap((item) => {
            const resolved = item.startsWith("~")
              ? join(homedir(), item.slice(1))
              : item;
      ...
      await Promise.all(
        Array.from(this.runtimes.values()).map((managed) =>
          managed.runtime.session.reload(),
        ),
      );
