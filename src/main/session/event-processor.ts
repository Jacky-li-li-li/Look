  ...
  handle(sessionId: string, event: AgentSessionEvent): void {
    ...
    if (!scope) {
      ...
      if (process.env.NODE_ENV === "development") {
        console.warn(
          `[Look][EventProcessor] scope not found for ${sessionId.slice(0, 6)}, dropping ${event.type}`,
        );
      }
      return;
    }
    ...
    const uiEvents = translateAgentSessionEvent(
      event,
      scope.translationTracker,
    );
