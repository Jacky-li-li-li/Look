...
interface PlanHost {
	getMode: () => "plan" | "ask" | "always";
	askQuestions: ReturnType<typeof vi.fn>;
	submitPlan: ReturnType<typeof vi.fn>;
}
...
function setup(mode: "plan" | "ask" | "always" = "plan") {
	const tools: Array<{
		...
		execute: (
			...args: unknown[]
		) => Promise<{ content: { text: string }[]; details?: Record<string, unknown>; terminate?: boolean }>;
	}> = [];
	...
	createPlanExtensionFactory(
		...
		registerTool: (tool: unknown) => tools.push(tool as (typeof tools)[0]),
		on: (name: string, handler: unknown) => handlers.set(name, handler as (...args: unknown[]) => unknown),
		sendMessage: sendMessage as unknown as (...args: unknown[]) => unknown,
	} as unknown as ExtensionAPI);
	return { tools, handlers, sendMessage: sendMessage as unknown as (...args: unknown[]) => unknown, host };
}
...
describe("Plan extension", () => {
	...
	it("rejects duplicate question text before opening an interaction", async () => {
		...
		expect((result as { content: { text: string }[] }).content[0].text).toContain("Duplicate question text");
	...
	it("validates question count, header length, and option count in the executor", async () => {
		...
		for (const questions of [
			...
			expect((result as { content: { text: string }[] }).content[0].text).toContain("Error:");
		}
		expect(host.askQuestions).not.toHaveBeenCalled();
	});
	...
	it("returns original questions and structured answers", async () => {
		...
		})) as { details: { answers: Record<string, string>; questions: { header: string }[] } };
