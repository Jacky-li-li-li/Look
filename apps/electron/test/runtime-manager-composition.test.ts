...
const source = readFileSync(resolve(import.meta.dirname, "../src/main/session/runtime-manager-composition.ts"), "utf8");

describe("RuntimeManagerComposition", () => {
	it("constructs agent definitions before injecting them into the subagent service", () => {
		const definitionIndex = source.indexOf("self.agentDefinitionService = new AgentDefinitionService");
		const subagentIndex = source.indexOf("self.sessionSubagentService = new SessionSubagentService");
